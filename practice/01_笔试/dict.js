...
位置1:'b'
此时比较的是:bda和dab的字典序列,
  - 字典中的字符串都是不重复的所以此时比较b:tempstr[index]和d:tempstr[index+1]的字典序列就可以直接判定dba和dab的字典序列,
  - 字典序列的比较是从左到右进行比较的且是不重复的,当删除b:tempstr[index]的字符时,其后一位的d:tempstr[index+1]字符就是第二位,即
